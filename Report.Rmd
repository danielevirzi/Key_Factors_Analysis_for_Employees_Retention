...
The second goal of this analysis is to predict the attrition of employees. For this purpose, we will build a classification model and, to evaluate the performance of the model, we will use the R-squared metric. In this case, we can use defiance instead of RSS. This, because we can treat the deviance in almost the same way as the residual sum of squares in linear models, as a measure of the goodness of fit of the model. Since the target variable is binary, we will use the logistic regression model. The main idea is to start with all the variables and then remove the variables that are not significant. The best model will be the one with the highest R-squared value and the variables with the highest significance.
...
```{r logistic_regression_summary, echo=TRUE}
...
```{r stepwise_selection_summary, echo=TRUE}
...
initial_predictions <- predict(model_slr, newdata = test.data)
final_predictions <- predict(final_model, newdata = test.data)
initial_mse <- mean((test.data$YearsAtCompany - initial_predictions)^2)
final_mse <- mean((test.data$YearsAtCompany - final_predictions)^2)
```

```{r regression_evaluation_summary, echo=FALSE}
print(paste("MSE of the initial model on test set:", initial_mse))
print(paste("MSE of the final model on test set:", final_mse))
```

```{r residuals, echo=TRUE, fig.width=10, fig.height=6}
residuals <- test.data$YearsAtCompany - final_predictions
par(mfrow = c(1, 1))
...
initial_prob_logit <- predict(initial_model_logit, newdata = test.data, type = "response")

initial_predictions_logit <- rep("No", 860)
initial_predictions_logit[initial_prob_logit>0.5] <- "Yes"

final_prob_logit <- predict(final_model_logit, newdata = test.data, type = "response")

final_predictions_logit <- rep("No", 860)
final_predictions_logit[final_prob_logit>0.5] <- "Yes"

initial_perf <- perf.measure(test.data$Attrition, initial_predictions_logit, lab.pos = "Yes")
final_perf <- perf.measure(test.data$Attrition, final_predictions_logit, lab.pos = "Yes")
```

We compare the performance of the initial logistic regression model with the final logistic regression model.
...
```{r confusion_matrix, echo=TRUE, fig.width=10, fig.height=6}
conf_matrix <- table(final_predictions_logit, test.data$Attrition)
conf_matrix
```

We plot the ROC curve of the final logistic regression model, compute the AUC and find the best threshold.

```{r roc_curve, echo=TRUE, fig.width=10, fig.height=6}
roc.out <- roc(test.data$Attrition, final_prob_logit, levels=c("No", "Yes"))
plot(roc.out,  print.auc=TRUE, legacy.axes=TRUE, xlab="False Positive Rate", ylab="True Positive Rate")
auc(roc.out)
coords(roc.out, "best")
```

The final logistic regression model has a better performance than the initial model. The confusion matrix shows that the final model has a higher number of true positives and true negatives. The ROC curve shows that the final model has a higher AUC than the initial model.

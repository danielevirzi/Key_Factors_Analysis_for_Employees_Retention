...
We found `MonthlyIncome` to be right-skewed, thus we applied a log transformation to normalize its distribution.
...
QDA is a more flexible model than LDA, as it does not assume that the covariance of the predictors is the same in each class. Thus, we tried QDA to see if we can achieve a performance improvement.
...
Naive Bayes is a probabilistic classification algorithm based on Bayes’ Theorem, which assumes independence between the features.
...
To conclude our analysis, we provide an overview of the performance of the models we developed.  Specifically, we demonstrate the significant improvement of our final regression model compared to the initial one. Additionally, we deliver an in-depth analysis on the different classification models using a variety of different metrics. Given the imbalance in our dataset regarding the attrition variable, we conducted an additional analysis on the classification models using a balanced dataset, placing greater emphasis on the F1-Score.
...
The final model demonstrates a lower MSE and a higher R-squared value compared to the base model. Therefore, the final model is superior and should be used for predicting the attrition rate of employees. Furthermore, due to the residuals being distributed randomly around 0, the superior model's predictions are unbiased.


## 8.2 Comparison of the Classification Models

We showcase the ROC curves of all our classification models.
...
We give a comparison of the classification models based on various evaluation metrics.

```{r classification_evaluation_comparison, echo=TRUE}
...
## 8.3  Comparison of the Classification Models with Balanced Data

...
## 4.5 Encode Key and Mode

We have noticed that the columns **`key`** and **`mode`** were *factors* and we wanted to encode them into *numeric* values. We have used the `model.matrix()` function to encode the columns. Moreover, we have noticed a missing key in the column so we have replaced the blank spaces in the **`key`** column with the correct key **`C`**. We have checked the distribution of the keys and then we have added the encoded columns to the dataset. Finally, we have dropped the original columns.

```{r encode}
# Replace blank spaces in the 'key' column with 'C'
Spotify$key <- sub("^$", "C", Spotify$key)

# Check the distribution of the keys
table(Spotify$key)

# Encode key and mode columns
encoded_key <- model.matrix(~ key - 1,
                            data = Spotify)

encoded_mode <- model.matrix(~ mode - 1,
                             data = Spotify)

# Add the encoded columns to the dataset
Spotify <- cbind(Spotify,
                 encoded_key,
                 encoded_mode)
...
# Now, create a histogram and a density plot for each column but the last 14
for(col in names(numerical_Spotify)[1:(num_of_columns-14)]) {
...
# Loop through each numerical column but the last 14
for(col in names(numerical_Spotify)[1:(num_of_columns-14)]) {
...
# Compute the correlation matrix for the numerical columns but the last 14
cor_matrix <- cor(numerical_Spotify[1:(num_of_columns-14)],
                  use = "complete.obs")
...
## 5.2 Top 10 artists with the most streams

```{r top_artists, fig.width=15, fig.height=10}
# Get the top 10 artists with the most streams in descending order
group_by_artists <- aggregate(streams ~ main_artist,
                              data = spotify_with_splitted_artists,
                              FUN = sum)

top_10_artists <- group_by_artists[order(group_by_artists$streams,
                                         decreasing = TRUE),
                                   ][1:10,]

# Reverse the order of the rows so that the artist with the most streams is on top
top_10_artists <- top_10_artists[nrow(top_10_artists):1, ]

# Plot the top 10 artists with the most streams

# Set up the graphics window
par(mar=c(5, 10, 5, 5))

# Create a horizontal barplot with the highest on top and the lowest at the bottom
barplot(top_10_artists$streams,
        names.arg = top_10_artists$main_artist,
        horiz = TRUE,
        las = 1,
        col = "lightblue",
        xlab = "Streams",

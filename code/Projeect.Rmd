...
for(col in names(numerical_Spotify)) {
  # Create a histogram with probability densities instead of counts on the y-axis
  hist(numerical_Spotify[[col]],
       breaks = 50,
       prob = TRUE,  # Show densities instead of counts
       main = paste(col),
       xlab = col,
       ylab = "Density",
       lwd = 0.5,
  ...
  density_data <- density(numerical_Spotify[[col]], na.rm = TRUE)

...
write.csv(Spotify_with_splitted_artists, "Spotify_with_splitted_artists.csv")


```

## 4.5 Encoding artist names

We have noticed that the column **`main_Artist`** and **`feature_1`** to **`feature_7`** were *categorical* and we wanted to convert them into *factors* using `as.factor()`.
...
```{r pairplot, fig.width=15, fig.height=20}
